...
class ParseIgnoredLabelsTests(unittest.TestCase):
    def test_empty_value_disables_ignored_labels(self):
        self.assertEqual(bump_semver.parse_ignored_labels(""), set())

    def test_normalizes_labels(self):
        self.assertEqual(
            bump_semver.parse_ignored_labels(" Dependencies, Needs-Review "),
...
class ResolveBumpFromLabelsTests(unittest.TestCase):
    def test_returns_none_when_only_ignored_labels_exist(self):
        result = bump_semver.resolve_bump_from_labels(
            10,
    ...
    def test_rejects_multiple_semver_labels(self):
        with self.assertRaisesRegex(bump_semver.ActionError, "Multiple version labels"):
            bump_semver.resolve_bump_from_labels(
                12,
...
class BumpFromPreviousTests(unittest.TestCase):
    ...
    def test_rejects_invalid_bump(self):
        with self.assertRaisesRegex(bump_semver.ActionError, "Unsupported version bump"):
